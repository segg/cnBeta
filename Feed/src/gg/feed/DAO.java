...
public class DAO {
	...
	public static void updateNewsList(String id) {
		...
		Objectify ofy = ObjectifyService.begin();
		Query<News> newss = ofy.query(News.class).filter("id <= ", id)
				.order("-id").limit(DEFAULT_SIZE);
		StringBuffer sb = new StringBuffer();
	...
	public static void updateNewsList() {
		...
		Objectify ofy = ObjectifyService.begin();
		Query<News> newss = ofy.query(News.class).order("-id")
				.limit(DEFAULT_SIZE);
		StringBuffer sb = new StringBuffer();
		String firstArticleId = "0";
		for (News news : newss) {
	...
	public static Set<String> getArticleIds() {
		log.info("getArticleIds");
		String html = fetchHtml(BASE_URL, "UTF8");
		Document doc = Jsoup.parse(html);

		Element realtimeList = doc.getElementsByClass("all_news_wildlist").get(
				0);
		...
		for (Element e : links) {
			...
			String id = href.replaceAll(".*/", "").replace(".htm", "");
			if (!id.isEmpty()) {
				linkset.add(id);
			}
		}
	...
	public static boolean updateNewss() {
		try {
			...
			Set<String> articleIds = getArticleIds();
	...
	public static News fetchNews(String id) {
		try {
			String html = fetchHtml(BASE_URL + "/articles/" + id + ".htm",
					"UTF8");
			Document doc = Jsoup.parse(html);
			...
			Element outterContent = doc.getElementsByClass("article_content")
					.get(0);
			Element intro = outterContent.getElementsByClass("introduction")
					.get(0);
			...
			description = description.substring(0,
					Math.min(400, description.length()));
